...
from sognet.dataset.json_dataset import JsonDataset, extend_with_flipped_entries, filter_for_training, add_bbox_regression_targets
from sognet.dataset.base_dataset import BaseDataset
from sognet.rpn.assign_anchor import add_rpn_blobs
...
class coco(BaseDataset):
    ...
    def __init__(self, image_sets, flip=False, proposal_files=None, phase='train', result_path=''):
        ...
        anno_files = {
            ...
            'panoptic_train2017': 'panoptic_instances_train2017.json',
            'panoptic_val2017': 'panoptic_instances_val2017.json',
        }
        ...
        if phase == 'train' and len(image_sets) > 1:
            ...
            for image_set, proposal_file in zip(image_sets, proposal_files):
                dataset = JsonDataset('coco_' + image_set,
                                      ...
                                      pan_ins_file=os.path.join(config.dataset.dataset_path, 'annotations', anno_files['panoptic_' + image_set]))
            ...
            self.dataset = JsonDataset('coco_' + image_sets[0],
                                       ...
                                       pan_ins_file=os.path.join(config.dataset.dataset_path, 'annotations', anno_files['panoptic_' + image_sets[0]]))
    ...
    def __getitem__(self, index):
        ...
        if config.network.has_fcn_head:
            if self.phase != 'test':
                ...
                label.update({'matched_mask': np.zeros((len(label['gt_classes']), im_blob.shape[-2], im_blob.shape[-1]))})
                idx = 0
                for i in range(len(label['roidb']['gt_classes'])):
                    if label['roidb']['is_crowd'][i] != 0:
                        continue
                    label['mask_gt'][idx] = self.decode_mask(label['roidb']['segms'][i], im_blob_shape=im_blob.shape, im_scales=im_scales)
                    if label['roidb']['matched_segms'][i] is not None:
                        label['matched_mask'][idx] = self.decode_mask(label['roidb']['matched_segms'][i], im_blob_shape=im_blob.shape, im_scales=im_scales, pan=True)
    ...
    def decode_mask(self, mask, im_blob_shape, im_scales, pan=False):
        if pan:
            output = mask_util.decode(mask)
            decoded_mask =  cv2.resize(output, None, None, fx=im_scales[0], fy=im_scales[0], interpolation=cv2.INTER_NEAREST)
            return decoded_mask
        if type(mask) is list and type(mask[0]) is list:
            img = Image.new('L', (int(np.round(im_blob_shape[-1] / im_scales[0])), int(np.round(im_blob_shape[-2] / im_scales[0]))), 0)
            for j in range(len(mask)):
                ImageDraw.Draw(img).polygon(tuple(mask[j]), outline=1, fill=1)
            decoded_mask = cv2.resize(np.array(img), None, None, fx=im_scales[0], fy=im_scales[0], interpolation=cv2.INTER_NEAREST)
        else:
            assert type(mask) is dict or type(mask[0]) is dict
            if type(mask) is dict:
                decoded_mask = cv2.resize(mask_util.decode(mask_util.frPyObjects([mask], mask['size'][0], mask['size'][1]))[:, :, 0], \
                    None, None, fx=im_scales[0], fy=im_scales[0], interpolation=cv2.INTER_NEAREST)
            else:
                assert len(mask) == 1
                output = mask_util.decode(mask)
                decoded_mask = cv2.resize(output[:, :, 0], None, None, fx=im_scales[0], fy=im_scales[0], interpolation=cv2.INTER_NEAREST)
